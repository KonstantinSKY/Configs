...
1. Copy the monitor config to your home directory:
   ```bash
   mkdir -p ~/.config/i3/host
   cp ~/Work/Configs/i3/desktop/monitor.conf ~/.config/i3/host/monitor.conf
   ```

2. Reload i3: Press `Mod+Shift+R`

### Method 2: Edit the Monitor Config
Edit `~/.config/i3/host/monitor.conf` and uncomment your preferred resolution:
...
1. Make sure the monitor config is in the correct location:
   ```bash
   ls ~/.config/i3/host/monitor.conf
   ```
...
3. If the include line is missing, add it to your i3 config:
   ```
   include ~/.config/i3/host/monitor.conf
   ```
...
- `~/.config/i3/host/monitor.conf` - i3 monitor configuration
